...
**Current release: v0.39.0.** Draftpaper-loop now adds a tiered review authority layer, activity-backed checkpoint narratives, and longitudinal drift governance on top of the existing evidence-first workflow. New projects default to `balanced`: notification-only stages may continue automatically; C1 requires a valid, hash-bound Agent delegation; C2 additionally requires explicit scientific-freeze permission and an independent reviewer Agent; C3 scientific choices and final release remain human-only. An Agent receipt is always `agent_approved`, never `user_confirmed`. Checkpoint packages use `dpl.checkpoint_summary.v4` and include a `StageActivityBundle` that produces a fact-backed stage narrative of what the Agent actually read, analyzed, generated, modified, reused, validated, retried, skipped, or failed. Immutable scientific baselines, revision cycles, canonical fact IDs, managed edits, and reconciliation packets protect repeated revisions from silently changing facts or mixing versions. The wheel remains local-first: pypdf is the default parser, official MinerU Agent is optional, and self-hosted MinerU/GPU infrastructure is not installed automatically. See [Recent Updates](#recent-updates) for the release history; the rest of this README is organized by research task.
...
- `v0.39.0` aligns source, wheel, Skill copies, schema registry, CLI contracts, README, and release manifest. The release manifest records 242 registered commands, v4 checkpoint companions, v2/v3 trace and transaction contracts, and cross-revision governance.
