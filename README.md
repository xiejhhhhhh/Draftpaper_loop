...
- **Supplementary tables and truthful command outcomes.** Method execution permits CSV, TSV, and TeX tables under `supplementary/tables/`. CLI results are emitted only after transaction checks finish; rejected writes report rollback status and a nonzero exit code without also publishing the handler's success result. Table output permissions do not replace scientific validation or evidence registration.
