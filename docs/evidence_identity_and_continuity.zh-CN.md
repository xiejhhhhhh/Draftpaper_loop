...
v1-v4 checkpoint 成果包始终是不可变的历史记录。`audit-checkpoint-v5-migration` 可以只读
检查它们，并可为 v4 建立比较预览，但绝不改写成果包，也不会把旧 receipt 当作 v5 批准。若
无法与已经核验的 v5 决定证明等价，下一条有效路线就是一次显式的 v5 C3 作者确认。

`shadow-checkpoint-v5` 只能使用项目目录外的输出目录。它会核验 v5 投影，并在只读审计前后
对项目状态取 hash。shadow 报告只是回归证据，不是项目产物，也不能替代作者确认。
