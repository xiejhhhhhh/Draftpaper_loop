...
## Data 与 Methods 已执行细节的证据绑定

如果修订文字说明某项数据处理、训练或分析步骤已经执行，需要从当前项目 manifest 中加入 `evidence_refs`。Preview 只读取固定的本地证据 allowlist，并输出 `suggested_evidence_refs`；用户选择与当前 run、cohort 和 snapshot 一致的候选即可。

```yaml
evidence_refs:
  - artifact: methods/run_manifest.yaml
    json_pointer: /steps/feature_construction
    expected_sha256: <suggested_evidence_refs 给出的 sha256>
    run_id: <当前 run id>
    cohort_id: <当前 cohort id>
```

ref 缺失、过期、跨 run、跨 cohort 或跨 snapshot 时会返回 `classification_refinement_required`。Apply 会在 preview 之后再次校验这些 ref；manifest 发生变化时需要重新 preview。
