...
Live search uses Semantic Scholar, arXiv, Crossref, and optional SerpApi, and can expand through OpenAlex, PubMed, Europe PMC, DBLP, and optional NASA ADS. Zotero collections and registered local PDF/BibTeX/RIS/JSON sources are user-curated inputs. See `docs/literature_sources.md` for source preservation, provider degradation, coverage review, and optional MinerU parsing.
