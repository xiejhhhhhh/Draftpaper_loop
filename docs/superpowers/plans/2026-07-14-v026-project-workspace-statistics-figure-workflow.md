...
### 2.4 人工确认后的研究蓝图是关键图表的唯一权威合同

研究蓝图与可行性确认不是一般性的流程提示，而是关键图表生成的科学授权边界。用户确认后的 plan snapshot 必须成为后续数据补充、方法实现、分析代码、主图 panel、统计检验和结果解释共同读取的唯一权威合同。
...
如果确认后的蓝图存在科学设计错误，应把问题和影响展示给用户，由用户纠错或确认修改。系统生成新的 plan version 和 hash 后，必须重新经过研究蓝图与可行性确认，才能继续生成关键图表。
...
关键图表生成器只能读取 `confirmed_research_blueprint_snapshot.json` 中的 figure storyboard、claim contract、data/method requirements 和 statistical validation contract。未进入确认快照的草案字段、通用 fallback 和学科默认图组不得进入主图代码生成。
...
生成后执行逐项 alignment check。只要研究问题、变量角色、样本单位、方法输出、panel 结构或统计设计与确认快照不一致，该图就不能进入核心结果确认，也不能由 Agent 自动寻找近似图替代。
...
-> freeze confirmed research blueprint snapshot
...
- plan hash 和显式确认记录。
- confirmed research blueprint snapshot 与严格执行锁。
...
6. 每个主图及其 panel 都携带 confirmed plan hash，并严格对应已确认的 claim、storyboard、数据角色、方法输出和统计验证。
...
15. Agent 不得在确认后静默更换方法、数据角色、主图或 panel；任何科学合同变化都会重新打开 research plan 并等待人工纠错和确认。
