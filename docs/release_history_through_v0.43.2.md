# Historical Release Notes Through v0.43.2

[Current README](../README.md) | [中文归档](release_history_through_v0.43.2.zh-CN.md)

These version-by-version notes were moved from the README on 2026-09-24. They preserve the original release context, including historical test counts and verification claims; they are not a statement of current test status. Later main-branch updates and consolidated milestones are maintained in the README.
...
- Ecology, environmental science, agriculture, and public-health markers join the ontology; `general` is split into explainable unknown/general states. A symmetric conflict policy blocks both astronomy-to-ecology and ecology-to-astronomy false recall, while explicit method transfer remains reviewable/context-only.
- Every shortlist candidate receives a DOI/title/author/year identity receipt. `ambiguous` and `mismatch` never fetch automatically. The default `resolve_then_fetch_on_demand` policy fetches only missing or declared evidence, uses one batch command for three or more papers, disables images/supplements, and reuses stable identity and full-text hashes.
- Resolved metadata and text are rescored after fetch. Only `accepted_active` and `accepted_context_only` enter the active snapshot; mismatches, insufficient evidence, and historical orphans stay in quarantine. Agent context, summaries, citation pools, and read counts consume active-snapshot-reachable artifacts only.
- Bilingual offline HTML now exposes pipeline totals, discovery provider, identity status, full-text decision, post-fetch status, citation eligibility, quarantine reason, and recovery route. Historical orphan handling is preview → packet hash → apply → hash-verified rollback.
- The frozen cross-discipline benchmark covers astronomy/ecology, medicine/stellar survival, remote sensing/astronomical satellites, digital-village/network governance, machine-learning generic terms, and materials science. Positive recall is 100% and hard-negative active count is zero. Core ships a pinned vendored paper-fetch adapter; installing the external Agent skill is not required.

## 2026-08-18 -- Multi-source literature integrity hardening

- The literature core now uses an augment-first merge path: adding online, Zotero, structured, manual, or local-PDF sources preserves existing work identity, provenance, scores, summaries, and code-source links unless an explicit replace transaction is requested.
- Added `literature_work_registry.json`, snapshot and merge receipts, score status/provenance, `audit-literature-integrity`, `sync-literature-sources`, `apply-literature-sync`, `repair-literature-identities`, local-only `migrate-literature-index` with rollback, resumable `parse-literature-source-documents`, `rebuild-literature-index`, and hash-checked orphan-fulltext quarantine/rollback. Missing scores are shown as `not evaluated`; legacy all-zero rows are shown as `legacy zero ambiguous`, never silently treated as scientific zeros.
- The Core literature pages now provide one offline `中文 | English` switch for the index and detail pages, while sharing the same work set, numbers, parser/binding states, and snapshot identity.
- Local PDF reconciliation records a formal parser state, evidence passages, `work_id` binding, and registry update. pypdf remains the default route; MinerU remains a quality-triggered optional upgrade.
- The real `EuclidxDESI` literature index is a read-only structural regression sample: the current input manifest records 26 active rows, 14 `quick_read`, 5 `pdf_available_unreadable`, 7 `not_parsed`, and 16 unmatched fulltext artifacts. These observed counts are recorded from the input snapshot rather than hard-coded into Core behavior; all migration/apply checks run only in an isolated copy.

## v0.37.1-v0.39.0 (2026-08-11) -- Delegated Review, Stage Narratives, and Longitudinal Drift Governance

- `v0.37.1` records `dpl.workflow_trace.v2` and `dpl.command_transaction.v3` activity facts, generates a shadow authority report, and introduces immutable scientific-baseline and revision-cycle records without changing legacy project decisions.
- `v0.38.0` promotes checkpoint packages to `dpl.checkpoint_summary.v4`. The first screen now contains a fact-backed Chinese paragraph explaining the Agent's actual work, followed by action-level receipts, artifact changes, validation, unresolved issues, review authority, and baseline references. The complete artifact inventory remains available for inspection.
- `v0.38.0` adds `manual`, `balanced`, and `delegated` review modes, scoped delegation files, `evaluate-checkpoint-authority`, `review-checkpoint`, and `agent_approved` receipts. `continue` can consume only a valid delegated C1/C2 review; C2 additionally requires explicit scientific-freeze permission and an independent reviewer Agent. An Agent can never impersonate a user or pass a C3 gate.
- `v0.38.1` adds `CanonicalFactRegistry v2`, immutable `ScientificBaselineBundle`, `RevisionCycle`, managed edit packets, `dpl.drift_reconciliation.v2`, cross-revision consistency reports, and routes for rebuild, expected-change adoption, or scientific reopening.
- `v0.39.0` aligns source, wheel, Skill copies, schema registry, CLI contracts, README, and release manifest. That release manifest records 242 registered commands, v4 checkpoint companions, v2/v3 trace and transaction contracts, and cross-revision governance.
- Verification: focused governance regression, compileall, Ruff, command contracts, release manifest, and isolated workflow smoke tests pass. Full release acceptance must still run the repository's complete pytest, wheel, and platform matrix before a formal tag is published.
- Boundary: fixtures, shadow reports, Agent review receipts, and HTML packages validate workflow contracts and provenance; they are not scientific conclusions and do not replace author confirmation of protected decisions.
...
- The v0.37.0 stage-package, code-source, and acceptance records are documented in [Human Checkpoint Packages](human_checkpoints.md), [Research Code Sources](research_code_sources.md), and the [Definition of Done audit](quality/v0.37.0_definition_of_done_audit.md).
