...
当前版本为 v0.31.6。跨学科 fixture 只验证通用流程合同，不代表真实科研结果。真实论文仍必须使用 live-runnable 学科插件或可审计的 project-local 代码，产生经过验证的 run output，并由用户确认研究蓝图、核心证据、最终作者补全 packet 和发布 hash。Mock 与 fixture 不能冒充论文证据。
...
## 最近更新
### v0.31.6（2026-07-18）-- 显式安装档位

- 默认 wheel 现在只安装工作流、参考文献和 PDF/图像检查所需的 minimal runtime，不再静默安装 NumPy、pandas 或 Matplotlib。绘图、增强全文抽取和 MCP 使用独立 extra；科研插件延迟导入 NumPy，Doctor 会报告档位可用性和恢复命令，CI 分别安装验证 minimal/plotting/fulltext/MCP 环境。新增 wheel metadata verifier 防止可选运行栈重新漂入核心依赖，同时 minimal 安装仍保留打包的 paper-fetch fallback。

### v0.31.5（2026-07-18）-- 资源 Schema 注册与质量门增强

- Release fixture、capability pack、命令合同、release manifest、方法运行/公式 manifest 和图表评估现已使用登记过的 schema family。发布验证会检查 wheel 内资源的 schema，不再仅凭 JSON 结构看似合理就放行。CI 的第一方 coverage 门从 45% 提升到 65%，Pyright 范围扩展至命令控制面、completion/stale 路径、figure façade 和拆分后的 Methods 模块。

### v0.31.4（2026-07-18）-- 单一命令控制面

- 全部 209 个 CLI 命令现在都先进入 `CommandSpec`；`legacy_dispatch_count` 已归零，`cli.py` 不再保留命令专用 fallback 链。尚未拆为 typed handler 的命令进入显式 namespace compatibility adapter，其数量继续公开而不是伪装成迁移完成。Pipeline stage 命令由 registry metadata 生成，`assess-figure-contracts` 也已改用统一 figure façade。

### v0.31.3（2026-07-18）-- 方法职责与图表合同 façade

- 方法执行验证、公式提取和 Methods 正文写作已拆为独立职责模块，并继续通过稳定的 `draftpaper_cli.methods` API 对外提供。新增 figure contract façade，将 gate、语义、人工确认蓝图和 caption 检查统一为确定性的 `{source, code, severity, message}` issue list；旧 run manifest 字符串字段继续兼容，新调用方读取 `normalized_issues`。

### v0.31.2（2026-07-18）-- 插件候选职责包

- 原先超过 3700 行的 plugin candidate 单体文件已拆为 skill source 读取、能力提取、受控 promotion 和 contribution 审核四个模块。既有 `draftpaper_cli.plugin_candidates` 导入路径及受审计 registry hook 保持兼容；专门的包结构测试禁止单体文件回归，并持续验证提取、AcademicForge metadata、provenance、promotion、fixture 和 contribution 行为不变。

### v0.31.1（2026-07-18）-- 共享工具与生产 ID 合同

- DPL 稳定 claim/evidence ID 已接入真实的 claim contract 和 Scientific Evidence Registry fallback，不再只是测试辅助函数；项目显式 ID 仍保持权威。当前 claim、evidence、reference 与 Data 写作热路径统一使用共享 JSON/文本 IO、引用解析和 LaTeX 转义，同时从 artifact DAG 删除了未参与运行的并行假依赖图。
