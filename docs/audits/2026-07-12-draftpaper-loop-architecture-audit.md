...
- 当时的质量声明曾使用 `prepare-blind-quality-evaluation` 与 `record-blind-quality-evaluation`。该 A/B 比较协议已在 v0.24.0 被独立单稿审查取代：两位 reviewer 只审查同一份冻结匿名生成稿，不再接收原稿，也不计算相对质量比例。自动分数仍不能单独授权发布。
