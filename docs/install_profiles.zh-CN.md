...
| Browser | `python -m pip install "draftpaper-cli[browser]"` | 可选的出版社浏览器回退；Python 3.11/3.12，浏览器资产需单独安装 |
| 科研工作站 | `python -m pip install "draftpaper-cli[plotting,fulltext]"` | 合并的本地科研环境；Agent/MCP 需要额外安装 `mcp` |
...
Python 档位不等于完整的论文生产环境。本地编译最终稿还需要可用的 TeX 发行版及 `xelatex`、`pdflatex`、`bibtex`、`kpsewhich`；源码 checkout 还需要可独立发现的系统 Git。`minimal`/`plotting` 支持 Python 3.10-3.12；`fulltext`/`research`/`publication`/`agent`/`browser` 支持 Python 3.11-3.12。Windows 的 MiKTeX 25.12 私有安装、Visual C++ x64 运行库、只读诊断和隔离编译验收见[完整环境部署手册](environment_deployment.zh-CN.md)。

发布 CI 会分别验证 minimal、plotting、fulltext、MCP 和 browser 独立环境；fulltext 还必须真实导入 vendored paper-fetch CLI。`mineru-agent` 是不增加依赖的 connector 档位。通过 `tools/verify_install_matrix.py` 检查 wheel metadata，防止重量级绘图或 GPU 包静默进入默认依赖。自建 MinerU 属于用户自行维护的外部 endpoint，不是安装档位。
