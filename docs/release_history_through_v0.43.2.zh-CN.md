...
[当前 README](../README.zh-CN.md) | [English archive](release_history_through_v0.43.2.md)

以下逐版记录于 2026-09-24 从 README 完整迁入，保留原始发布语境。历史测试数量与验证声明只描述当时的版本，不代表当前验收状态；之后的 main 更新与合并后的能力里程碑维护在 README 中。
...
- 在保留完整不可变审阅产物的同时限制 Agent 决策载荷，并改善可行性恢复动作、标题回退、重复数据清单抑制和 Windows reparse point 安全遍历。
- 区分派生输出与缺失源数据，避免监督学习的 `partition` 声明错误触发无监督验证要求。
- 将 EP/WXT、AGN/XRB、current/history/spectral 文件标记、光谱处理角色和专用中文翻译迁入天文学/时域模块。
- 为复合学科 profile 增加确定性的扩展合并，并用回归测试保证天文学语义只在天文学 profile 激活，不泄漏到默认或无关学科。
...
- 决定页中重复出现的 artifact evidence 改为精确且可复制的路径定位符，主交付物和图表仍保留链接。这样同一 canonical artifact 支撑多条事实或陈述时，不会仅因重复链接超过预算而阻断原本有效的确认页。
- figure 声明的紧凑表格只有在 cohort、estimand、run、model、split 与 analysis binding 完整时，才能以 content-addressed 形式进入证据 registry。cohort identity 现在参与 conflict key，不同 cohort 的同一指标不再被误报为冲突。
- core-evidence continuity 现在包含稳定的可执行分析规格与方法合同身份。分析规格、选择或重采样合同、声明实现入口或其项目内代码 hash 变化都会要求新的 C3；文字、引用映射、HTML 和 PDF 呈现变化不会。缺少该身份的旧核心证据 v5 包仍可阅读，但只能走迁移与新建 C3 路线。
...
- v0.37.0 的阶段成果包、代码来源合同和逐项验收记录分别见[人工确认点成果包](human_checkpoints.zh-CN.md)、[科研代码来源](research_code_sources.zh-CN.md)和[Definition of Done 审计](quality/v0.37.0_definition_of_done_audit.md)。
...
- wheel 回归扩展为 scientific-image astronomy+ML、geography+ML、time-domain astronomy+ML、bioinformatics/medicine 和 physics/quantum 五类场景，并验证任务感知统计合同和规则缺口显式报告。当前源码验证为 `565 passed`；完整的 15 项验收证据见 [`docs/audits/2026-07-14-v0260-acceptance-report.md`](audits/2026-07-14-v0260-acceptance-report.md)。
...
- wheel 内 held-out 发布回归扩展为 astronomy+ML、Euclid 语境 geography+ML、bioinformatics+medicine，以及此前未作为发布夹具的 physics+quantum，并保留 scope、图表、插件 runtime 和 citation 对抗检查。机器可读闭环账本见 [`docs/audits/v025_issue_ledger.json`](audits/v025_issue_ledger.json)；41 条 AcademicForge 集合级 placeholder 继续标记 `requires_source_inspection`，不会进入插件充分性。
...
- 使用 `tiktoken` `o200k_base` 统计可观察写作消耗：章节证据包输入 122,238 tokens，接受稿输出 5,716 tokens，共 127,954 tokens；Results 40,075、Introduction 15,691、Data 17,139、Methods 22,920、Discussion 32,129。确定性 CLI 阶段不消耗模型 tokens，隐藏推理与平台开销不作估算。完整问题链与统计见 [`docs/audits/2026-07-12-astronomy-v0230-full-run.md`](audits/2026-07-12-astronomy-v0230-full-run.md)。
