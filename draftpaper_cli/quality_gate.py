...
def _check_scientific_evidence_registry(project_path: Path, issues: list[QualityIssue]) -> dict[str, Any]:
    report = check_scientific_evidence_registry(project_path)
...
def _check_citation_audit(project_path: Path, issues: list[QualityIssue]) -> dict[str, Any]:
    ...
    binding = report.get("manuscript_snapshot") if isinstance(report, dict) else {}
    if binding:
        try:
            validate_citation_audit_snapshot(project_path, binding)
            snapshot_status = "current"
        except EvidenceSnapshotMismatch as exc:
            snapshot_status = "stale"
            issues.append(QualityIssue(
                "error",
                "citation_audit_stale_after_manuscript_change",
                str(exc),
                "citation_audit/final_citation_audit_report.json",
            ))
    elif status == "passed":
        issues.append(QualityIssue(
            "error",
            "citation_audit_snapshot_missing",
    ...
    if expected_evidence_snapshot_id and audited_evidence_snapshot_id != expected_evidence_snapshot_id:
        issues.append(QualityIssue(
            "error",
            "citation_audit_evidence_snapshot_mismatch",
            "The final citation audit does not cover the promoted scientific evidence snapshot used by the manuscript.",
            "citation_audit/final_citation_audit_report.json",
        ))
    return {
        "status": status,
...
def run_quality_check(project: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        evidence_snapshot_status = "current" if promoted_snapshot else "legacy_unpromoted"
    except EvidenceSnapshotMismatch as exc:
        promoted_snapshot = {}
        evidence_snapshot_status = "stale_or_missing"
        issues.append(QualityIssue(
            "error",
            "promoted_evidence_snapshot_invalid",
            str(exc),
    ...
    scientific_evidence_report = _check_scientific_evidence_registry(state.path, issues)
    ...
    report = {
        ...
        "evidence_snapshot": {
            ...
            "snapshot_id": promoted_snapshot.get("snapshot_id"),
        },
        "stages": stage_report,
        "data": data_report,
        ...
        "scientific_evidence": scientific_evidence_report,
