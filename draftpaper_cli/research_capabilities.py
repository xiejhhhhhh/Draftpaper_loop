...
from .capability_packs import discover_capability_packs, route_capability_requirement
...
# These values are generated by the planned analysis. Requiring them as
...
def _extract_requirements(project_path: Path, profile: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for index, figure in enumerate(figures, start=1):
        ...
        data_roles = figure.get("required_data_roles") or figure.get("required_data") or figure.get("data_roles") or []
        ...
        for role in data_roles:
            ...
            requirements.append({
                ...
                "claim_ids": claim_ids,
                "role": role,
                "data_role_class": role_class,
                "availability_stage": "post_method" if role_class == "derived_method_output" else "pre_method",
                "core": is_main,
                "discipline": primary,
                "required_outputs": [role],
            })
        families = figure.get("method_families") or figure.get("required_method") or figure.get("method_family") or method_requirements.get("method_families") or []
...
def resolve_research_capabilities(project: str | Path) -> dict[str, Any]:
    ...
    routed_requirements = [
        ...
        if item.get("kind") in {"data", "method", "review"}
    ]
    selected_pack_ids = sorted(
        {
            str(item.get("selected_pack_id"))
            for item in routed_requirements
    ...
    discipline_contract = {
        ...
        "capability_pack_ids": selected_pack_ids,
        "capability_pack_routing": routed_requirements,
    }
    capability_contract = {
        "status": "written",
        "generated_at": utc_now(),
...
def _assess_requirement(
    ...
    if kind == "data" and requirement.get("data_role_class") == "derived_method_output":
        return {
            **requirement,
            ...
            "matched_plugin": None,
            "match_score": 0,
            "match_reasons": ["generated_by_bound_method_run"],
            "runtime_class": None,
            "validation_level": None,
            "runtime_level": None,
            "coverage_basis": "post_method_output_contract",
            "capability_pack_id": None,
            "capability_pack_routing_status": "deferred_to_method_run",
            "alternatives": [],
        }
    candidates = catalog[kind]
    pack_route = route_capability_requirement(
        requirement,
        primary_discipline=str(profile.get("primary_discipline") or "default"),
        secondary_disciplines=[str(item) for item in profile.get("secondary_disciplines") or []],
    )
    pack_plugins = set()
    if isinstance(pack_route.get("selected"), dict):
        pack_plugins = set((pack_route["selected"].get("plugin_ids") or {}).get(kind) or [])
    scored = []
    for candidate in candidates:
        score, reasons = _structured_match(requirement, candidate)
        if _candidate_id(candidate, kind) in pack_plugins:
            score += 8
            reasons.append("capability_pack_binding")
    ...
    if not selected:
        state = "project_method_implementation_required" if kind == "method" else "project_data_implementation_required"
    else:
    ...
    return {
        **requirement,
        ...
        "capability_pack_id": pack_route.get("selected_pack_id"),
        "capability_pack_routing_status": pack_route.get("status"),
...
def assess_plugin_sufficiency(project: str | Path) -> dict[str, Any]:
    ...
    implementation_required = [
        item
        for item in core
        if item.get("state") in {"project_method_implementation_required", "project_data_implementation_required"}
    ]
    rescue_required = [
        item
        for item in core
        if item.get("state") not in {
            ...
            "project_data_implementation_required",
            "method_output_pending",
        }
    ]
    rescue_tasks = [{
        ...
        "state": item["state"],
        "reason": "A required core-figure capability needs a project implementation or scoped rescue.",
        "recommended_command": (
            "prepare-project-method-implementation"
            if item.get("state") == "project_method_implementation_required"
            else "prepare-data-acquisition"
            if item.get("state") == "project_data_implementation_required"
            else "prepare-plugin-rescue"
        ),
        "search_scope": {"discipline": item.get("discipline"), "role": item.get("role") or item.get("method_family")},
    } for item in implementation_required + rescue_required]
    bindings = [{
        "requirement_id": item["requirement_id"],
        "figure_id": item.get("figure_id"),
        "kind": item["kind"],
    ...
    decision = (
        "rescue_required"
        if rescue_required
        else "project_implementation_required"
    ...
    report = {
        ...
        "blocking_count": 0,
        "implementation_required_count": len(implementation_required),
    ...
    return {"status": "written", "project_path": str(state.path), "decision": report["decision"], "core_figure_decision": report["core_figure_decision"], "blocking_count": 0, "report": SUFFICIENCY_REPORT}
