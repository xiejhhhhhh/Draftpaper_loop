...
class DiscussionWriterTests(unittest.TestCase):
    ...
    def test_write_discussion_creates_latex_with_traceable_citations(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Figure~\\ref", tex)
            self.assertNotIn("current draft should", tex)
            self.assertNotIn("manuscript should", tex)
            self.assertNotIn("Future revisions should", tex)
            self.assertNotIn("Figure\\textasciitilde{}", tex)
            ...
            self.assertNotIn("results/figures", tex)
            self.assertNotIn("risk_curve.png", tex)
            self.assertNotIn("metrics.csv", tex)
