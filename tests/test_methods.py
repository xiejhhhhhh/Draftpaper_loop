...
class MethodsHardGateTests(unittest.TestCase):
    ...
    def test_write_methods_expands_time_aware_classification_formulas_and_sanitizes_internal_terms(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("\\subsection{Sample Construction and Observation Products}", tex)
            self.assertIn("\\subsection{Temporal, Spectral, and Tabular Representation}", tex)
            self.assertIn("\\subsection{Classifier Objective, Validation, and Ablation}", tex)
